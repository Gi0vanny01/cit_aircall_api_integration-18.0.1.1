...
class AircallWebhook(models.TransientModel):
    ...
    def create_call_detail(self, call_id, recording, waiting_time, i, mail_data, data,
        ...
        last_parent = parents[-1] if parents else i

        air = self.env['aircall.details'].sudo().create({
            'aircall_call_id': call_id,
            ...
            'call_duration': time.strftime('%H:%M:%S', time.gmtime(data['duration'])),
    ...
    def _process_call_with_conf_num(self, payload, comments, tags):
        ...
        if not check_msg and payload.get('event') == 'call.ended' and external_entity_id:
            ...
            if partner:
                if not manual_call_loging or partner.add_log_note:
    ...
    def _generate_message(self, data, talk_time, waiting_time, tags, comments):
        ...
        return Markup(
            ...
            'Aircall Number': '{}'.format(data.get("number", {}).get("name", "")),
    ...
    def _post_message_and_create_call(self, partner, message, data, talk_time, waiting_time,
        ...
        if not manual_call_loging and len(partner_matches) > 1:
            ...
            print("Found parent contacts:", parents)

            if parents:
                last_parent = parents.sorted(lambda p: p.create_date, reverse=True)[0]  # Get the last created parent
                print(f"Posting message to last created parent: {last_parent.name}")
                mail_data = last_parent.sudo().message_post(
                    body=message,
                    author_id=author_id
                )
            else:
                last_created_contact = partner_matches.sorted(lambda p: p.create_date, reverse=True)[0]
                mail_data = last_created_contact.sudo().message_post(
                    body=message,
                    author_id=author_id
                )
        else:
        ...
        if mail_data:
            ...
            partner.sudo().write({'add_log_note': False})  # Reset add_log_note
    ...
    def _populate_insight_card(self, payload):
        ...
        if manual_call_loging:
            ...
            parents = partner.mapped('parent_id').filtered(lambda p: p)
            if parents:
                last_parent = parents[-1]
                partner_name_string = last_parent.name
        else:
    ...
    def _generate_insight_card_json(self, partner, partner_name_string, data):
        ...
        if len(partner) == 1:
            # Single partner: pass the partner's id
            base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url') + \
                       '/web?=#action=%s&model=crm.lead&view_type=form&cids=1&id=%s' % (
                       partner_open.id, partner.id)
        else:
            # Multiple partners: no specific id passed, use list view
            base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url') + \
